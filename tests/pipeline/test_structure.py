...
import fastr_python.pipeline.markers as pipeline_markers
import fastr_python.pipeline.provenance as pipeline_provenance
from fastr_python.pipeline import PipelineInputError
from fastr_python.pipeline.models import PipelineInputError as SharedPipelineInputError
...
def test_pipeline_public_boundary_has_no_processing_logic() -> None:
    from fastr_python.pipeline import (
        CorrectionSummary,
        PipelineInputError,
        run_correction,
    )
    from fastr_python.pipeline.models import CorrectionSummary as SummaryModel
    from fastr_python.pipeline.runner import run_correction as implementation

    assert CorrectionSummary is SummaryModel
    assert run_correction is implementation
    assert issubclass(PipelineInputError, ValueError)
...
def test_marker_helper_matches_the_existing_pipeline_behavior() -> None:
    ...
    markers = pipeline_markers.bad_gradient_markers(
        spans,
        window=window,
        decimation=1,
    ...
    assert markers[0].description == "Bad_Gradient"
