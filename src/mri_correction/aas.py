...
def slice_group_aas(
    data: npt.ArrayLike,
    group_starts: npt.ArrayLike,
    *,
    samples_per_group: int,
    neighbor_count: int = 20,
) -> npt.NDArray[np.float64]:
    """Subtract local slice-group templates without using the target in its template."""
    recording = np.asarray(data)
    ...
    corrected = recording.astype(np.float64, copy=True)
    window_size = neighbor_count + 1
    rolling_sum = sum(
        (
            _baselined_epoch(recording, start, samples_per_group)
            for start in starts[:window_size]
        ),
        start=np.zeros((recording.shape[0], samples_per_group), dtype=np.float64),
    )
    current_window_start = 0
    half_window = window_size // 2
    last_window_start = starts.size - window_size
    ...
    for target_index, target_start in enumerate(starts):
        ...
        while current_window_start < window_start:
            outgoing = starts[current_window_start]
            incoming = starts[current_window_start + window_size]
            rolling_sum -= _baselined_epoch(recording, outgoing, samples_per_group)
            rolling_sum += _baselined_epoch(recording, incoming, samples_per_group)
            current_window_start += 1

        target_epoch = _baselined_epoch(
            recording,
            target_start,
            samples_per_group,
        )
        template = (rolling_sum - target_epoch) / neighbor_count
        target_slice = slice(target_start, target_start + samples_per_group)
        corrected[:, target_slice] = recording[:, target_slice] - template
...
def _validate_group_inputs(
    ...
    *,
    ...
    if not np.issubdtype(data.dtype, np.number) or not np.all(np.isfinite(data)):
        raise CorrectionInputError("data must contain only finite numeric values")
    if starts.ndim != 1 or not np.issubdtype(starts.dtype, np.integer):
        message = "group starts must be a one-dimensional integer array"
        raise CorrectionInputError(message)
    if not isinstance(samples_per_group, int) or samples_per_group < 1:
        raise CorrectionInputError("samples_per_group must be a positive integer")
    if not isinstance(neighbor_count, int) or neighbor_count < 2:
        raise CorrectionInputError("neighbor_count must be an integer of at least two")
    if neighbor_count % 2:
        raise CorrectionInputError("neighbor_count must be even")
    if starts.size < neighbor_count + 1:
        raise CorrectionInputError("not enough groups for the requested neighbor count")
    if starts[0] < 0 or np.any(np.diff(starts) < samples_per_group):
        message = "group epochs overlap or are not strictly increasing"
        raise CorrectionInputError(message)
    if starts[-1] + samples_per_group > data.shape[1]:
        raise CorrectionInputError("the final group epoch extends beyond the recording")
